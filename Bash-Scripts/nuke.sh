...
# Runs the script to reset an org to a snapshot
	...
	if [ -f cameras.setting ]; then
		...
		do
			...
			# The assumption that the last thread will have the same runtime as the previous threads
			LT=${!} # Get the pid of the last thread
			wait ${LT} # Wait for the thread to finish
		...
		do
			...
			# The assumption that the last thread will have the same runtime as the previous threads
			LT=${!} # Get pid of the last thread
			wait ${LT} # Wait for the threaad to finish
